...
- (void)closeOpenResultSets {
    ...
    for (NSValue *wrapped in openSetCopy) {
        FMResultSet *rs = (FMResultSet *)[wrapped pointerValue];
        
        [rs setParentDB:nil];
        [rs close];
        
        [_openResultSets removeObject:wrapped];
    }
}
...
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    NSAssert(_shouldCacheStatements, @"you should call -[FMDatabase setShouldCacheStatements:YES] before this method");
...
- (int)changes {
    ...
    // 获得有多少记录受到影响，提供最后一次执行的更新、插入、删除语句影响的记录数目。不包含原始命令触发器的结果
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (![self databaseExists]) return nil;
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            id object = [dictionaryArgs objectForKey:dictionaryKey];
            
            if (_traceExecution) {
                NSLog(@"%@ = %@", parameterName, object);
            }
            ...
            if (namedIdx > 0) {
                ...
                [self bindObject:object toColumn:namedIdx inStatement:pStmt];
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (SQLITE_DONE == rc || rc == SQLITE_OK) {
...
- (BOOL)rollback {
    BOOL b = [self executeUpdateWithSql:@"rollback transaction"];
...
- (BOOL)commit {
    BOOL b =  [self executeUpdateWithSql:@"commit transaction"];
...
- (BOOL)beginTransaction {
    ...
    BOOL b = [self executeUpdateWithSql:@"begin exclusive transaction"];
    if (b) {
...
- (BOOL)beginImmediateTransaction {
    ...
    BOOL b = [self executeUpdateWithSql:@"begin immediate transaction"];
    if (b) {
        _isInTransaction = YES;
    }
